...
if who | grep -q "$orcaghost"; then
    echo "User $orcaghost is currently logged in. Logging out user..."
    sudo pkill -u "$orcaghost" || { echo "Failed to kill processes owned by $orcaghost"; exit 1; }
fi
# Rename orca-ghost
echo "Renaming $orcaghost to $num..."
echo "$password" | sudo usermod -l "$num" "$orcaghost" || { echo "Failed to rename user"; exit 1; }
echo "$password" | sudo usermod -d "/home/$num" -m "$num" || { echo "Failed to move home directory"; exit 1; }
echo "$password" | sudo ln -s  "/home/$num" "/home/$orcaghost" || { echo "Failed to create symlink"; exit 1; }

# Update the python environment or other configurations as needed
echo "Updating python environment..."
echo "$password" | sudo -S sed -i "s/$orcaghost/$num/g" "/home/$num/.pyenv/versions/3.7.4/bin/pip" || { echo "Failed to update python version"; exit 1; }
